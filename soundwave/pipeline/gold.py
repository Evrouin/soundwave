...
class GoldBuilder:
    ...
    def validate(self) -> None:
        ...
        for table, pk in checks.items():
            df = DeltaTable(f"{Paths.GOLD_BASE}/{table}", storage_options=self._opts).to_pandas()
    ...
    def _write(self, table_name: str, df: pd.DataFrame) -> int:
        ...
        write_deltalake(f"{Paths.GOLD_BASE}/{table_name}", df, mode="overwrite", storage_options=self._opts)
        return len(df)
    ...
    def _build_dim_artist(self, artists: pd.DataFrame) -> int:
        ...
        dim_artist["artist_sk"] = dim_artist.apply(
            lambda r: hashlib.md5(f"{r['artist_id']}_{r['valid_from']}".encode()).hexdigest(), axis=1
        )
    ...
    def _build_dim_genre(self, tracks: pd.DataFrame) -> int:
        ...
        dim_genre = pd.DataFrame(
            {"genre_id": [hashlib.md5(g.encode()).hexdigest() for g in genres], "genre_name": genres}
        )
        return self._write("dim_genre", dim_genre)

    def _build_fact_streams(self, tracks: pd.DataFrame, artists: pd.DataFrame) -> int:
        genre_dt = DeltaTable(f"{Paths.GOLD_BASE}/dim_genre", storage_options=self._opts).to_pandas()
        genre_map = dict(zip(genre_dt["genre_name"], genre_dt["genre_id"]))

        current = artists[artists["is_current"]]
        artist_id_map = dict(zip(current["artist_name"], current["artist_id"]))

        fact = tracks[["track_id", "popularity", "track_genre", "artists", "ingestion_timestamp"]].copy()
        ...
        fact["stream_id"] = fact.apply(
            lambda r: hashlib.md5(f"{r['track_id']}_{r['ingestion_date']}".encode()).hexdigest(), axis=1
        )
        fact_streams = fact[
            ["stream_id", "track_id", "artist_id", "genre_id", "ingestion_date", "popularity", "mood_cluster"]
        ]
        return self._write("fact_streams", fact_streams)

    def _build_agg_genre_trends(self, tracks: pd.DataFrame) -> int:
        agg = (
            tracks.groupby("track_genre")
            .agg(
    ...
    def _build_agg_artist_profiles(self, tracks: pd.DataFrame, artists: pd.DataFrame) -> int:
        current = artists[artists["is_current"]]
        artist_id_map = dict(zip(current["artist_name"], current["artist_id"]))
        ...
        agg = (
            df.groupby(["_first_artist", "_first_artist_id"])
            .agg(
                track_count=("track_id", "count"),
                avg_popularity=("popularity", "mean"),
                avg_danceability=("danceability", "mean"),
            ...
            .reset_index()
            .rename(columns={"_first_artist": "artist_name", "_first_artist_id": "artist_id"})
        )
